(function () {
  ...
  function renderFamilies() {
    ...
    familyOrder.forEach(family => {
      ...
      block.style.cssText = "margin:12px 0;padding:12px 14px;border-radius:8px;border-left:4px solid " + familyColors[family] + ";";
      const fn = el("strong","");
      fn.style.cssText = "font-size:14px;color:" + familyColors[family] + ";display:block;margin-bottom:4px;";
  ...
  async function renderQuestion() {
    ...
    if (chatSource) {
      ...
      cc.querySelectorAll('.mwai-conversation, .mwai-chat').forEach(m => {
        const mc = m.querySelector('.mwai-messages'); if (mc) mc.innerHTML = '';
      });
  ...
  function renderDISCOptions(card, q) {
    ...
    [{label:"Completely Disagree",value:1,color:"#d9534f",emoji:"👎"},
     ...
     {label:"Completely Agree",   value:5,color:"#4caf50",emoji:"💯"}
      ...
      btn.onmouseover = () => { btn.style.transform="translateY(-3px)"; btn.style.boxShadow="0 4px 12px rgba(0,0,0,0.3)"; };
  ...
  async function handleMBTIAnswer(question, choice, chosenLetter) {
    ...
    try {
      const res = await fetch(cfg.restUrlAnswer, {
        method:'POST', headers:{'Content-Type':'application/json','X-WP-Nonce':cfg.nonce||''}, credentials:'same-origin',
        body: JSON.stringify({ week, question_id: question.id, q_type:'MBTI', answer:choice, letter:chosenLetter, axis:question.mbti_axis })
      });
  ...
  async function handleDISCAnswer(question, answerValue) {
    ...
    try {
      ...
      const res = await fetch(cfg.restUrlAnswer, {
        method:'POST', headers:{'Content-Type':'application/json','X-WP-Nonce':cfg.nonce||''}, credentials:'same-origin',
        body: JSON.stringify({
          week, question_id:question.id, q_type:'DISC', answer:answerValue,
  ...
  /* ================================================================
     RESULTS SCREEN 2 — Tabbed summary + DISC + chatbot
     ================================================================ */
  function renderSummaryDetail() {
    ...
    if (sd.mbti_type) {
      ...
      mt1.textContent = "The " + p.name;
      miniText.appendChild(mt1);
      const mt2 = el("div","");
      mt2.style.cssText = "font-size:13px;color:" + p.familyColor + ";font-weight:600;font-family:var(--font-heading);text-transform:uppercase;letter-spacing:0.04em;";
      mt2.textContent = p.family + " family";
    ...
    if (hasDiscScores) {
      ...
      const dt = el("h3",""); dt.style.cssText = "margin:0 0 16px;font-size:20px;text-align:center;";
    ...
    if (chatSource) {
      ...
      const cpt = el("p",""); cpt.style.cssText = "margin:0;font-weight:600;";
  ...
  function createDISCPolarPlot(scores) {
    ...
    ctx.strokeStyle='rgba(0,212,255,0.15)';ctx.lineWidth=2;ctx.setLineDash([5,5]);
    ...
    ctx.setLineDash([]);
    segs.forEach(s=>{const p=scores[s.key]||0,fr=(p/100)*mr;ctx.fillStyle=cols[s.key];ctx.globalAlpha=0.65;ctx.beginPath();ctx.moveTo(cx,cy);ctx.arc(cx,cy,fr,s.sa,s.ea);ctx.closePath();ctx.fill();ctx.globalAlpha=1;ctx.strokeStyle=cols[s.key];ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(cx,cy);ctx.arc(cx,cy,mr,s.sa,s.ea);ctx.closePath();ctx.stroke();});
    ctx.font='bold 14px Arial';ctx.fillStyle='#E8EAF0';ctx.textAlign='center';
    ...
    segs.forEach(s=>{const p=scores[s.key]||0,ma=(s.sa+s.ea)/2;
      ctx.font='bold 48px Arial';ctx.fillStyle='#E8EAF0';ctx.textAlign='center';ctx.fillText(s.key,cx+mr*0.4*Math.cos(ma),(cy+mr*0.4*Math.sin(ma))+15);
      ctx.font='bold 16px Arial';ctx.fillStyle='#E8EAF0';ctx.fillText(s.label,cx+(mr+50)*Math.cos(ma),cy+(mr+50)*Math.sin(ma));
      ctx.font='bold 16px Arial';ctx.fillStyle=cols[s.key];ctx.fillText(Math.round(p)+'%',cx+mr*0.65*Math.cos(ma),(cy+mr*0.65*Math.sin(ma))+5);
    });
    return canvas;
  }
